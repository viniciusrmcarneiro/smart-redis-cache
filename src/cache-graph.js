...
const getEntity = graph => ({ key, entity }) => {
    return graph
        ...
        .then(res => res.next())
        .then(obj => (obj !== undefined ? toJSON(obj.keys(), obj.values()) : undefined))
        .catch(err => {
...
const discardEntityByKey = graph => ({ entity, key }) => {
    ...
    return getEntity(graph)({ key, entity }).then(cached => {
        if (cached) {
            return graph.query(`MATCH (obj:${entity}) WHERE obj._key = '${key}' DELETE obj`);
        }
    });
};

const publishChangedEntity = discardEntityByKey;
const graph = new RedisGraph('cache-vinny', { host: process.env.REDIS_SERVER, port: 6379 });
module.exports = {
